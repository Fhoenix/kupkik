...
public class HtmlRequestProcessor
{
    ...
    public final static UserWithPassword GUEST_USER = new UserWithPassword("guest", "none", KeyFactory.createKey("User", "guest"), "guest",
                                                            "guest");
    ...
    private void doHandleClientRequest() throws Exception
    {
        ...
        // prepare the data IMPORTANT: For all data needed in the view, such a
        // Handler
    ...
    private IHtmlRequestHandler loadRequestHandlerByName( final String pNameOfRequestHandler ) throws Exception
    {
        ...
        JarFile jarFile = new JarFile(pathToCodeBase);
        Enumeration<JarEntry> e = jarFile.entries();

        URL[] urls = { new URL("jar:file:" + pathToCodeBase + "!/") };
        URLClassLoader classLoader = URLClassLoader.newInstance(urls);

        while( e.hasMoreElements() )
        {
            JarEntry currentJarEntry = e.nextElement();
            if( currentJarEntry.isDirectory() || !currentJarEntry.getName().endsWith("Handler.class") )
            {
                continue;
            }

            String expectedName = "com/kupkik/ui/html/requesthandlers/" + pNameOfRequestHandler + "Handler.class";
            if( !currentJarEntry.getName().equals(expectedName) )
            {
                continue;
            }
            ...
            final Class htmlRequestHandlerClass = classLoader.loadClass(className);
        ...
        // final File requestHandlersDirectory = new File(pathToCodeBase +
        // "com/kupkik/ui/html/requesthandlers");
        //
        // final ClassLoader classLoader =
        // HtmlStarterServlet.class.getClassLoader();
        //
        // for( File currentFile : requestHandlersDirectory.listFiles() )
        // {
        // String className = currentFile.getName().substring(0,
        // currentFile.getName().length() - 6);
        // String fullClassName = "com.kupkik.ui.html.requesthandlers." +
        // className;
        ...
        // {
        //
        // final Class htmlRequestHandlerClass =
        // classLoader.loadClass(fullClassName);
        // final IHtmlRequestHandler htmlRequestHandler = (IHtmlRequestHandler)
        // htmlRequestHandlerClass.newInstance();
        // sLogger.info("Found request-handler with name: " +
        // pNameOfRequestHandler);
        // return htmlRequestHandler;
        // }
        // }
        //
        // sLogger.info("Found no request-handler with name: " +
        // pNameOfRequestHandler);
        // return null;
    }
    ...
    private boolean doesViewExist( final String pViewName ) throws Exception
    {
        ...
        File viewsDirectory = classesDirectory.getParentFile().getParentFile().getParentFile();
